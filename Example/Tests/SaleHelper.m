...
{
    WDAcceptSaleRequest *_currentSale;
    WDAcceptSaleResponse *_currentSaleResponse;
    NSString *_cashRegisterId, *_shopId, *_cashierId, *_customerId;
    NSString *_selectedCurrencyCode, *_loginCredentials;
    WDAcceptPaymentConfig *_paymentConfiguration;
...
-(WDAcceptSaleRequest *)newSale
{
    ...
    _currentSale = [[WDAcceptSaleRequest alloc] initWithUniqueId:transactionUniqueId
...
-(WDAcceptReceipt*)receiptForDatecsFromReceiptData:(WDAcceptReceiptData*)receiptData
{
    ...
    WDAcceptReceipt *receipt = [WDAcceptReceipt new];
    if (!receiptData)
    {
        return receipt;
    }
    ...
    if (receiptData.saleType == AcceptSaleTypeReturn )
    {
        receipt.saleTypeAsText =  NSLocalizedString(@"REFUND",@"REFUND");
    }
    else if (receiptData.status ==  AcceptSaleStateCompleted || receiptData.status ==  AcceptSaleStateReturned)
    {
        receipt.saleTypeAsText =  NSLocalizedString(@"SALE",@"SALE");
    }
    else if (receiptData.status == AcceptSaleStateFailed ||
             receiptData.status == AcceptSaleStateFailedIntervene ||
             receiptData.status == AcceptSaleStateUnknown)
    {
        receipt.saleTypeAsText =  NSLocalizedString(@"DECLINED",@"DECLINED");
    }
    else if (receiptData.status == AcceptSaleStateInProgress)
    {
        receipt.saleTypeAsText =  NSLocalizedString(@"INCOMPLETE",@"INCOMPLETE");
    }
    else if (receiptData.status == AcceptSaleStateIncomplete)
    {
        //Incomplete will always end as declined on backend in a matter of seconds.
        receipt.saleTypeAsText =  NSLocalizedString(@"DECLINED",@"DECLINED");
    }
    else if (receiptData.status == AcceptSaleStateCanceled)
    {
    ...
    {
        if (i.intValue == AcceptPaymentMethodCard)
        {
        ...
        else if (i.intValue == AcceptPaymentMethodCoupon)
        {
            if (paymentTypeStr.length > 1)
            {
                [paymentTypeStr appendString:@", "];
    ...
    NSString *transactionStatus = [receiptData receiptAcceptSaleStatusDescription];
    ...
    receipt.footer = [NSString stringWithFormat:@"%@ %@", NSLocalizedString(@"Payment issued by", @"Payment issued by"), @"AcceptDemo"];
    ...
    for (WDAcceptSaleItem *item in receiptData.receiptItems)
    {
        if (item.itemType == AcceptSaleItemTypePurchase)
        {
    ...
    for (WDAcceptSaleSummaryItem *item in receiptData.taxSummaryItems)
    {
