...
-(void)requestCatalogueData
{
    ...
    ProductCatalogueCompletion completionProductCatalogue = ^(NSArray *arrCatalogues, NSError *err)
    {
        ...
        for (WDAcceptProductCatalogue *catalogue in arrCatalogues)
        {
            if (successfulDownload)
            {
...
-(void)loadContentForCatalogue:(WDAcceptProductCatalogue*)catalogue
{
    ...
    ProductCatalogueCategoryCompletion completionCatalogue = ^(NSArray <WDAcceptProductCatalogueCategory *>* result, NSError* err)
    {
        ...
        WDAcceptProductsQuery *query = [WDAcceptProductsQuery new];
...
-(void)loadContentForProducts:(NSArray <WDAcceptProductCatalogueProduct*>*)products inCatalogueId:(NSString*)catalogueId andCategory:(NSString*)categoryId
{
    ...
    if ([[products firstObject] isKindOfClass:WDAcceptProductCatalogueProduct.class])
    {
        dispatch_group_enter(_serviceGroup);
        NSMutableArray *queries= [[NSMutableArray alloc] init];
        
        for (WDAcceptProductCatalogueProduct *product in products)
        {
        ...
        ProductCatalogueImageCompletion completionImage = ^(NSArray <WDAcceptProductImage *>*prodsImage, NSError *err)
        {
...
-(void)loadContentForCategory:(WDAcceptProductCatalogueCategory*)category inCatalogueId:(NSString*)catalogueId andVersion:(NSDecimalNumber*)dbVersion
{
    ...
    if ([category isKindOfClass:WDAcceptProductCatalogueCategory.class])
    {
        ...
        {
            if ([subcategory isKindOfClass: WDAcceptProductCatalogueCategory.class])
            {
                if (![category.categoryId isEqualToString:catalogueId] && ![[(WDAcceptProductCatalogueCategory*)subcategory parentCategoryId] length])
                {
