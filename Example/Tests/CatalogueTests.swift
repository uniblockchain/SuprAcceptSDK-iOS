...
class CatalogueTestsSwift: BaseTestsSwift
{
    ...
    func requestCatalogueData()
    {
        ...
        let completionProductCatalogue : ProductCatalogueCompletion = {[weak self](arrCatalogues : [WDAcceptProductCatalogue]?, error : Error?) in
    ...
    func loadContentForProducts(products : [WDAcceptProductCatalogueProduct]?, catalogueId : String, categoryId : String?)
    {
        if let products = products, products.first != nil
        {
            ...
            let completionImage : ProductCatalogueImageCompletion = {[weak self](prodsImage : [WDAcceptProductImage]?, error : Error?) in
                self?.serviceGroup.leave()
            }
