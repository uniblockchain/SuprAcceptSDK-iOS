...
class CashTestsSwift: BaseTestsSwift
{
    ...
    func getCashRegisters()
    {
        ...
        sdk.cashManager.cashRegisters(merchantId,
                                      ...
                                      completion: {[weak self](arrCashiers : [WDAcceptCashRegister]?, error : Error?) in
            ...
            if let cashregs : [WDAcceptCashRegister] = arrCashiers
            {
                for cashReg : WDAcceptCashRegister in cashregs
                {
    ...
    func getLastShift()
    {
        ...
        let query : WDAcceptShiftQuery = WDAcceptShiftQuery.init(page: 0,
                                                                 pageSize: 20,
                                                                 orderBy: .openTime,
                                                                 order: .descending)
        sdk.cashManager.shifts(cashRegisterId,
                               query: query,
                               completion: {[weak self](arrShifts : [WDAcceptCashShift]?, error : Error?) in
            self?.returnedErr = error
    ...
    func getCloseShiftDetailsThenOpen() //Used as a Z-Report example
    {
        ...
        sdk.cashManager.shiftDetails(lastShiftId,
                                     ...
                                     completion: {[weak self](cashShift : WDAcceptCashShift?, error : Error?) in
    ...
    func getShiftActivities()
    {
        ...
        sdk.cashManager.getOperationsForShift(lastShiftId,
                                              ...
                                              completion: {[weak self](arrActivities : [WDAcceptCashActivity]?, error : Error?) in
    ...
    func refundTransaction()
    {
        ...
        sdk.saleManager.refundSale(saleToBeRefunded,
                                   ...
                                   completion: {[weak self](acceptSale : WDAcceptSaleResponse?, error : Error?) in
